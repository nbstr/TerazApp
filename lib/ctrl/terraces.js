...
exports.around = function(req, res) {
	...
	var $p = {
		...
		limit:req.param('limit') ? parseInt(req.param('limit')) : 30,
		radius:req.param('radius') ? parseFloat(req.param('radius')) : 1000
	};

	function forecast (terraces_json){
		request('http://api.openweathermap.org/data/2.5/forecast?lat='+$p.lat+'&lon='+$p.lng+'&cnt=1&lang=fr&mode=json&units=metric', function (error, response, body) {
			if (!error && response.statusCode == 200) {
				...
				var $data = JSON.parse(body), $response = [], $css = '', $time = '';
				...
				for (var i = 0; i < 4; i++) {
					...
					else if ($data.list[i].weather[0].icon == '02d' || $data.list[i].weather[0].icon == '02n' || $data.list[i].weather[0].icon == '10d'){
						$css = 'cloudy';
					}
					else {
						$css = 'cloud';
					}
					if($data.list[i].dt_txt != undefined){
	...
	function terraces_around (radius){
		Terrace.aggregate([
			...
			else if (data.length < 3 && radius < $p.radius + (1000 * 5) + (500 * 5) ) {
				terraces_around (radius + 1000);
			}
			else if (data.length < 3 && radius < $p.radius + (1000 * 5) + (500 * 5) + (radius * 1024)) {
		      ...
		      console.error(error);
		    	...
		    	for (var line in result) {
		    		...
		    		*/
		    		...
		    		// VALIDATION
		    		if(!result[line][ 1]){ console.log('ohoh');continue; }
		    		if(!result[line][ 4]){ console.log('ohoh');continue; }
		    		if(!result[line][ 5]){ console.log('ohoh');continue; }
		    		if(!result[line][ 6]){ console.log('ohoh');continue; }
		    		if(!result[line][ 7]){ console.log('ohoh');continue; }
		    		if(!result[line][ 8]){ console.log('ohoh');continue; }
		    		// if(!result[line][ 9]){ console.log('ohoh');continue; }
