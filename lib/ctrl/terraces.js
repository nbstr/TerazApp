		    ...
		    if(error) {
		      console.error(error);
		    	...
		    	for (var line in result) {
		    		...
		    		if (result[line][12]) { new_data.sun_start = result[line][12]; }
		    		if (result[line][13]) { new_data.sun_end = result[line][13]; }
		    		if (result[line][11]) { new_data.sits = result[line][11]; }
		    		if (result[line][14]) { new_data.info = result[line][14]; }
		    		if (result[line][7]) { new_data.opening = result[line][7]; }

		    		if (result[line][8]) {
		    			new_data.phone = result[line][8];
		    			// CLEAR PARASITES
		    		};
		    		if (result[line][10]) {
		    			new_data.web = result[line][10];
		    			// ADD HTTP IF NECESSARY
		    		}
		    		if (result[line][9]) {
		    			new_data.mail = result[line][9];
